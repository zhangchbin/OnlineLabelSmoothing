...
from models.multi_resnext import ResNeXt29_2x64d, ResNeXt29_4x64d, ResNeXt29_32x4d
from models.iccv19_resnet_ds import *
from models.preactresnet import CIFAR_ResNet18, CIFAR_ResNet34 
from tensorboardX import SummaryWriter
from loss_all_methods import SCELoss, label_smooth, generalized_cross_entropy, joint_optimization, boot_soft, boot_hard, Forward, Backward, DisturbLabel
import random
import utils
import numpy as np
import cv2
...
class Solver():
    ...
    def update_loss_lams(self, output, target):
        with torch.no_grad():
            logits = torch.softmax(output / args.T, dim=1)
    ...
    def train(self, train_loader, epoch):
        ...
        for i, (input, target) in enumerate(train_loader):
            ...
            # compute output
            output, midout1, midout2 = self.model(input_var)
            ...
            if args.method == 'ols':
                ...
                if epoch == 0:
                    ...
                    loss = 0.5 * (1.5*self.criterion(output, target) + \
                               0.5 * self.criterion(midout1, target) + \
                               0.5 * self.criterion(midout2, target)) +\
                0.5 * (1.5*self.soft_cross_entropy(output, target)+\
                      0.25* self.soft_cross_entropy(midout1, target) +\
                      0.25* self.soft_cross_entropy(midout2, target))
            elif args.method == 'ls':
                if epoch == 0:
                    loss = 1.5 * torch.nn.functional.cross_entropy(output, target) + \
			    0.5 * torch.nn.functional.cross_entropy(midout1, target) + \
                            0.5 * torch.nn.functional.cross_entropy(midout2, target)
                else:
                    loss = 1.5 * self.criterion(output, target) + \
                            0.5 * self.criterion(midout1, target) + \
                            0.5 * self.criterion(midout2, target)
            else:
                loss = self.criterion(output, target)
