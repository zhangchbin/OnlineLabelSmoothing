...
class AVG_AGG(nn.Module):
    def __init__(self):
        super().__init__()
        self.weight = torch.tensor([[0, 0.2, 0], [0.2, 0.2, 0.2], [0, 0.2, 0]]).cuda()
        self.weight = self.weight.unsqueeze(0).unsqueeze(0)
    def forward(self, x):
        n, c, h, w = x.shape
        
        conv_weight = torch.zeros(c, 1, 3, 3).cuda()

        
        conv_weight += self.weight
        conv_weight.requires_grad = False
        out = nn.functional.conv2d(x, conv_weight, bias=None, stride=1, padding=1, groups=c)
        return out
...
class VGG(nn.Module):
    ...
    def forward_cam(self, x, target_layers=[0]):
        conv_output = []
...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == 'M':
            layers += [AVG_AGG(), nn.MaxPool2d(kernel_size=2, stride=2)]
        else:
